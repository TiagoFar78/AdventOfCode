...
public class Day15Part2 extends Challenge {
    ...
    private int solve(List<String> gridString) {
        ...
        while (l < r) {
            int m = (l + r) / 2;
            int temp = executeGame(gridString, m);
            if (temp < 0) {
                l = m + 1;
            }
            else {
    ...
    private int executeGame(List<String> gridString, int elfDamage) {
        int[][] grid = createGrid(gridString);
        int initialElves = elvesCount(grid);
        ...
        int rounds = 0;
        while (true) {
            while (!activeAgents.isEmpty() && hasBothTeamsAlive(grid)) {
                int[] agent = activeAgents.poll();
                play(grid, agent, agentsHP, elfDamage);
        ...
        return rounds * totalHP * (elvesCount(grid) != initialElves ? -1 : 1);
    }
    
    private int elvesCount(int[][] grid) {
        int n = grid.length;
        int m = grid[0].length;
        int count = 0;
        for (int i = 0; i < n; i++) {
    ...
    private boolean play(int[][] grid, int[] agent, int[] agentsHP, int elfDamage) {
        if (agentsHP[hpId(agent[2])] <= 0) {
            return hasBothTeamsAlive(grid);
        }
        
        return attack(grid, agent, agentsHP, elfDamage) || move(grid, agent, agentsHP, elfDamage);
    }
    ...
    private boolean attack(int[][] grid, int[] agent, int[] agentsHP, int elfDamage) {
        int targetId = -1;
        int line = -1;
        int col = -1;
        ...
        if (targetId != -1) {
            int damage = agent[2] > 0 ? elfDamage : GOBLIN_DAMAGE;
    ...
    private boolean move(int[][] grid, int[] agent, int[] agentsHP, int elfDamage) {
        ...
        return attack(grid, agent, agentsHP, elfDamage) || true;
    }
