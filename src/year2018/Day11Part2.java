package year2018;

import java.util.Scanner;

import main.Challenge;

public class Day11Part2 extends Challenge {
    
    private int[] solve(int serialNumber) {
        int[][] grid = new int[300][300];
        for (int x = 1; x <= 300; x++) {
            for (int y = 1; y <= 300; y++) {
                grid[x - 1][y - 1] = powerLevel(x, y, serialNumber);
            }
        }
        
        int maxPower = 0;
        int[] maxCell = new int[] { 0, 0, 0 };
        for (int size = 1; size <= 300; size++) {
            int[] totalPower = totalPower(grid, size);
            if (totalPower[2] > maxPower) {
                maxCell[0] = totalPower[0];
                maxCell[1] = totalPower[1];
                maxCell[2] = size;
                maxPower = totalPower[2];
            }
        }
        
        return maxCell;
    }
    
    private int[] totalPower(int[][] grid, int size) {
        int[] maxCell = new int[] { 0, 0, 0 };
        for (int x = 0; x < 301 - size; x++) {
            for (int y = 0; y < 301 - size; y++) {
                int totalPower = 0;
                for (int i = 0; i < size; i++) {
                    for (int j = 0; j < size; j++) {
                        totalPower += grid[x + i][y + j];
                    }
                }
                
                if (totalPower > maxCell[2]) {
                    maxCell[0] = x + 1;
                    maxCell[1] = y + 1;
                    maxCell[2] = totalPower;
                }
            }
        }
        
        return maxCell;
    }
    
    private int powerLevel(int x, int y, int serialNumber) {
        int rackId = x + 10;
        int powerLevel = rackId * y;
        powerLevel += serialNumber;
        powerLevel *= rackId;
        powerLevel = (powerLevel / 100) % 10;
        powerLevel -= 5;
        
        return powerLevel;
    }
    
    @Override
    public String solveString() {
        Scanner reader = getInputFile();
        int serialNumber = Integer.parseInt(reader.nextLine());
        reader.close();
        
        int[] cell = solve(serialNumber);
        return cell[0] + "," + cell[1] + "," + cell[2];
    }
    
    @Override
    public long solve() {
        // Empty
        return 0;
    }

}
