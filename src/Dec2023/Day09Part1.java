...
public class Day09Part1 {
	...
	public static void run() {
		...
		for (List<Integer> history : histories) {
			sum += getHistoryNextValue(history);
		}
		
		System.out.println(sum);
	}
	
	public static int getHistoryNextValue(List<Integer> history) {
		...
		for (int i = differenceSequences.size() - 2; i >= 0; i--) {
			...
			int newDifference = differenceSequece.get(differenceSequece.size() - 1) + differenceSequeceBellow.get(differenceSequeceBellow.size() - 1);
