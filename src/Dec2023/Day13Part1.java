...
public class Day13Part1 {
	...
	public static void run() {
		...
		for (List<String> a : verticalMirrorPatterns ) {
			for (String b : a) {
		...
		System.out.println("Lista hori:");
		for (List<String> a : horizontalMirrorPatterns ) {
			for (String b : a) {
				System.out.println(b);
			}
			System.out.println();
	...
	private static int getLinesBeforeMirror(List<String> pattern, boolean isVertical, int size, int startOffset, int endOffset) {
		for (int i = 0; ; i++) {
			...
			if (mirrowedLine - baseLine == 0 || mirrowedLine - baseLine == 1) {
				return baseLine + 1;
			}
		}
	}
