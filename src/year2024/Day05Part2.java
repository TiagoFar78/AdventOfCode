...
public class Day05Part2 extends Challenge {
    ...
    private int solve(List<int[]> rules, List<List<Integer>> pages) {
        ...
        for (List<Integer> page : pages) {
            if (!isValidPage(rulesMap, page)) {
                middleSum += getMiddleValue(rulesMap, page);
            }
        }
    ...
    private int getMiddleValue(Map<Integer, List<Integer>> rules, List<Integer> page) {
        page.sort((a, b) -> rules.get(b) != null && rules.get(b).contains(a) ? -1 : 1);
