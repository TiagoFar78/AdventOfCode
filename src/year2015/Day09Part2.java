...
public class Day09Part2 extends Challenge {
    ...
    private int solve(List<String[]> distances) {
        ...
        for (int i = 0; i < locations.size(); i++) {
            minDistance = Math.max(minDistance, calculateDistance(distancesMap, i, locations.size()));
        }
    ...
    private int calculateDistance(Map<String, Integer> distancesMap, List<Integer> seen, int max) {
        ...
        for (int i = 0; i < max; i++) {
            if (!seen.contains(i)) {
                ...
                minDistance = Math.max(minDistance, nextDistance + calculateDistance(distancesMap, newSeen, max));
            }
        }
