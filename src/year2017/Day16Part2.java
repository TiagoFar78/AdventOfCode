...
public class Day16Part2 extends Challenge {
    ...
    private static final int DANCES = 1_000_000_000;
    ...
    private String solve(String[] instructions) {
        ...
        int i = 0;
        while (!seen.containsKey(programs)) {
            seen.put(programs, i);
            i++;
        ...
        int dances = DANCES;
        dances -= seen.get(programs);
        int dancesLeft = dances % (i - seen.get(programs));
        for (i = 0; i < dancesLeft; i++) {
            programs = dance(programs, instructions);
        }
    ...
    private String dance(String programs, String[] instructions) {
